...
public:
    ...
    bool IsConstant;
    bool IsArray;
    std::vector<int> DimVec;            // 数组维度
    std::vector<float> DimVecf;            // 数组维度
    InitValTree<int>* InitVal;          // 整型初始化值
    InitValTree<float>* FloatInitVal;   // 浮点型初始化值
    int Int32Val;                       // 整型常量
    float Float32Val;                   // 浮点常量

    // 构造函数 1: 支持整型数组
    GlobalVarIRT(ValueType type, std::string name, bool ConstFlag, std::vector<int> dim, InitValTree<int>* init)
        : GlobalVarType(type), GlobalVarName(name), IsConstant(ConstFlag), IsArray(true), DimVec(dim), InitVal(init), FloatInitVal(NULL), Int32Val(0), Float32Val(0.0f) {
    }

    // 构造函数 2: 支持浮点型数组
    GlobalVarIRT(ValueType type, std::string name, bool ConstFlag, std::vector<float> dim, InitValTree<float>* init)
        : GlobalVarType(type), GlobalVarName(name), IsConstant(ConstFlag), IsArray(true), DimVecf(dim), InitVal(NULL), FloatInitVal(init), Int32Val(0), Float32Val(0.0f) {
    }

    // 构造函数 3: 支持整数常量
    GlobalVarIRT(ValueType type, std::string name, bool ConstFlag, int init)
        : GlobalVarType(type), GlobalVarName(name), IsConstant(ConstFlag), IsArray(false), InitVal(NULL), FloatInitVal(NULL), Int32Val(init), Float32Val(0.0f) {
    }

    // 构造函数 4: 支持浮点常量
    GlobalVarIRT(ValueType type, std::string name, bool ConstFlag, float init)
        : GlobalVarType(type), GlobalVarName(name), IsConstant(ConstFlag), IsArray(false), InitVal(NULL), FloatInitVal(NULL), Int32Val(0), Float32Val(init) {
    }
