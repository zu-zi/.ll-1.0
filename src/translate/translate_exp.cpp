...
if (primary_exp->tp == PrimaryType::NumInt || primary_exp->tp == PrimaryType::NumFloat)
{
    NumberAST *number = nullptr;
    if (primary_exp->number != nullptr)
    {
        number = dynamic_cast<NumberAST *>(primary_exp->number);
        if (primary_exp->tp == PrimaryType::NumInt)
        {
            ir = new ExpIRT(ExpKind::Const, new ConstIRT(number->intNum));
        }
        else
        {
            ir = new ExpIRT(ExpKind::Const, new ConstIRT(number->floatNum));
        }
    }
}
    ...
    else if (unary_exp->tp == ExpType::Call)
    {
        ...
        if (unary_exp->func_rparam == nullptr)
        {
            ...
            else if (sym->FunctionAttributes->RetValType == ValueType::FLOAT32) {
                ir = new ExpIRT(ExpKind::Call, new CallIRT(ValueType::FLOAT32, new LabelIRT(*(unary_exp->ident)), types, args,IsVarParaCount));
            } 
        ...
        else
        {
            ...
            CallParamsAST *func_r = dynamic_cast<CallParamsAST *>(unary_exp->func_rparam);
            ...
            for (auto &it : func_r->exp)
            {
                ...
                {
                    // if (args_exp->type == ValueType::FLOAT32) {
                    //     types.push_back(ArgsType::Float32);
                    // } else {
            ...
            if (sym->FunctionAttributes->RetValType == ValueType::INT32) {
                ...
                ir = new ExpIRT(ExpKind::Call, new CallIRT(ValueType::FLOAT32, new LabelIRT(*(unary_exp->ident)), types, args,IsVarCountPara));
            }
            else if (sym->FunctionAttributes->RetValType == ValueType::VOID) {
